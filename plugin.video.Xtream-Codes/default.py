...
#       V-----------------V This is a function that when a user logges in to reveal bellow directorys
        if active =="true":
            ...
            addDir('Coming Soon','%s:%s/panel_api.php?username=%s&password=%s'%(site,port,usern,passw),4,'')
...
def altervista(url):
        ...
        for url,name in match:
            addDir('%s'%(name),'%s'%url,10,'')
        for name,url in match2:
            addDir2('%s'%(name),'%s'%url,11,'')
...
def GRABEPG(url):
    ...
    if dialog.yesno("Download EPG", 'Do you Wish To Download EPG','', "",'Close','Yes'):
        ...
        url = 'http://thaisatellite.tv/ftv/guide.xmltv'
        localfile = os.path.join(addonDir,"resources/epg.xmltv")
